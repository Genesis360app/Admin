...
const BankingPage = () => {
  ...
  const fetchUserName = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE2_URL}/wallets/confirm/${userid}/${recieverName}`,
        {
          headers: {
  ...
  const sendMoney = async () => {
    ...
    if (userid && token) {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE2_URL}/wallets/send`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
                ...
                <div>
                  {showWithdrawButton ? (
                    <button
                      ...
                      disabled={
                        !userId || !recieverName || !amount || isLoading
                      }
                    >
                    ...
                    <>
                      ...
                      {firstname && (
                        <button
                          type="button"
                          className="btn btn-dark bg-gradient-to-r from-[#00b09b] from-10% via-sky-500 via-30% to-[#96c93d] to-90%"
                          onClick={sendMoney}
                          disabled={
                            !userId || !recieverName || !amount || isLoading
                          }
