...
const ExampleTwo = ({ item,title = "Advanced Table Two" }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/User/DashboardUser.php?userid=${userid}`, {
        headers: {
            "Authorization": `Bearer ${token}`
        }
        })
        ...
        if(res.code == 200){
          ...
          setAll_awaiting_payment(res.all_awaiting_payment);
          setAll_waiting_shipment(res.all_waiting_shipment);
          setAll_awaiting_delivery(res.all_awaiting_delivery);
